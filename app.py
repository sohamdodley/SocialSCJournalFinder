...
def load_journal_corpus():
    ...
    if not path.exists():
        st.error("`journal_rankings_final.csv` not found. Please upload it to the repository root.")
        return pd.DataFrame()
...
def extract_candidate_terms(text):
    """Simple term extraction from abstract."""
    text = text.lower()
    # Keep multi-word phrases and single words
    tokens = re.findall(r"[a-z][a-z\-]+(?:\s+[a-z][a-z\-]+){0,3}", text)
    # Filter very short / stop-ish
    candidates = []
    for t in tokens:
        t = t.strip()
        if len(t) < 4:
...
def quick_clean(candidates, vague):
    """Pathway A: remove vague terms."""
    cleaned = [c for c in candidates if c not in vague and not any(v in c.split() for v in VAGUE_TERMS)]
    return cleaned[:15]

def match_journals(keywords, mode="Conservative"):
    """Simple keyword overlap matching against corpus."""
    ...
    for _, row in journal_db.iterrows():
        ...
        overlap = [k for k in kw_set if k in text or any(tok in text for tok in k.split())]
        score = len(overlap)
    ...
    if mode == "Conservative":
        result = result[result["Strength"] >= 2]

    result = result.sort_values(["Strength", "Match_Score"], ascending=False)
...
if section == "1. Abstract Diagnosis":
    ...
    st.markdown("Paste a full abstract. The tool will extract candidate terms and flag common keyword problems.")

    abstract = st.text_area("Paste your abstract here", height=300,
...
# =========================================================
    ...
    if "diagnosis" not in st.session_state:
        ...
        if pathway.startswith("A"):
            ...
            st.markdown("Re-organise your terms into simple SPIDER components (fill what you can):")
...
# =========================================================
    ...
    if not keywords:
        ...
        st.caption("Conservative = journals appearing in multiple quality signals. Exploratory = broader list.")

        if st.button("Find Journals", type="primary"):
            results = match_journals(keywords, mode=mode)
            st.session_state["recommended_journals"] = results

            if results.empty:
                st.warning("No strong matches found. Try different keywords or use Exploratory mode.")
            else:
                st.subheader(f"Recommended Journals ({len(results)})")
                st.dataframe(
                    results[["Journal", "Category", "Quality_Level", "ABDC_Rank", "Strength", "Matched_Terms"]],
...
# =========================================================
    ...
    if "abstract" not in st.session_state:
        ...
        if not recs.empty:
            ...
            st.dataframe(
                recs[["Journal", "Category", "Quality_Level", "ABDC_Rank", "Strength"]].head(15),
                use_container_width=True,
                hide_index=True
            )

        st.subheader("Supervisor Feedback")
        kw_quality = st.selectbox("Keyword quality after improvement",
                                  ["Strong", "Adequate", "Needs work"])
        shortlist_quality = st.selectbox("Journal shortlist assessment",
                                         ["Suitable", "Too ambitious", "Too narrow", "Missing important journals"])
        intellectual_fit = st.selectbox("Intellectual fit",
                                        ["Strong", "Acceptable", "Weak"])
        confidence = st.selectbox("Confidence in the tool’s matching",
                                  ["High", "Medium", "Low"])
