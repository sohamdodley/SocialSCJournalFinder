...
# Simple mapping from app discipline → corpus category keywords
DISCIPLINE_MAP = {
...
def match_journals(keywords, discipline, mode="Conservative"):
    ...
    for _, row in journal_db.iterrows():
        ...
        discipline_bonus = 0
        if discipline_keys:
            if any(dk in category for dk in discipline_keys):
                discipline_bonus = 2
            elif any(dk in text for dk in discipline_keys):
                discipline_bonus = 1

        if score == 0 and discipline_bonus == 0:
            continue
        ...
        rows.append({
            ...
            "Strength": strength,
            "Matched_Terms": ", ".join(overlap[:5]) if overlap else "—",
            "Note": row.get("Note", "")
        })
    ...
    result = result.sort_values(["Strength", "Match_Score", "Discipline_Bonus"], ascending=False)
...
if section == "1. Abstract Diagnosis":
    ...
    st.markdown("Select your discipline and paste a full abstract. The tool will extract candidate terms and flag common keyword problems.")
...
# =========================================================
    ...
    discipline = st.session_state.get("discipline", "Interdisciplinary / Other Social Science")
    ...
    if not keywords:
        ...
        st.caption("Conservative = stronger multi-signal matches. Exploratory = broader list.")
